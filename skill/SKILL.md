...
- **Open on a specific, dated, quantified fact — never a stock big-question throat-clear.** Good: a named FOMC date and the basis-point move (Gürkaynak–Sack–Swanson). Not: "A central question in macroeconomics is…" — a stock opener even 9/10 writers fall into, and still slop. **Do not** confuse this with (a) a rhetorical-question pivot that immediately resolves into the paper's mechanism (provisional taste — allowed when the turn is real), or (b) an S8 section-frame question that states the job of *this* section. There is no unconditional "never open with a question."
